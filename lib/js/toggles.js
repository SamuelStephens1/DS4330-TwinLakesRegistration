document.addEventListener('DOMContentLoaded', function () {
        ...
        if (zip && phone) {
            lookup()
                .then(result => {
                    ...
                    const data = result.recordset; // Customer data is within `recordset`

                    if (numOfReturns > 0 && Array.isArray(data) && data.length > 0) {
                        if (data.length === 1) {
                            // Only one customer: go directly to ticket confirmation
                            ...
                            // Multiple customers: display selection
                            document.getElementById('divMemberLogin').style.display = 'none';
        ...
        customers.forEach((customer, index) => {
            ...
            for (const key in customer) {
                if (customer.hasOwnProperty(key)) {
                    if (key === 'member_number' || key === 'email') continue;
        ...
        if (name && address && county) {
            advlookup(name, address, county)
                ...
                .catch(error => {
                    ...
                    alert('An error occurred while fetching advanced search data. Please try again later.');
                });
        } else {
    ...
    // Advanced lookup API call function
    function advlookup(name, address, county) {
        const apiUrl = `http://localhost:3000/adv/${name}/${address}/${county}`;
        return fetch(apiUrl)
            .then(response => {
            ...
            .catch(error => {
                console.error('Error in Advanced Lookup API call:', error);
                throw error;
            });
    }
